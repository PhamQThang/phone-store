      ...
      <div className="hidden md:flex bg-red-500 items-center justify-center text-base font-semibold relative gap-5">
        {navItems.map((item) =>
          item.hasDropdown ? (
            ...
            >
              <span className="cursor-pointer text-2xl font-bold text-white transition-colors duration-200 flex items-center p-3 hover:bg-red-400 rounded-lg">
                {item.name}
              </span>
              {isProductsOpen && (
            ...
            <Link
              key={item.name}
              href={item.href}
              className="text-white font-bold text-2xl transition-colors duration-200 p-3 my-3 hover:bg-red-400 rounded-lg active:rounded-lg"
            >
              {item.name}
            </Link>
          )
